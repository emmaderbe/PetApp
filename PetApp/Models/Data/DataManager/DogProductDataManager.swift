...
extension DogProductDataManager {
    ...
    func filterProductsBySearchBar(searchText: String) -> [DogProductModel] {
        guard !searchText.isEmpty else { return getAllProducts() }
        return dogProduct.filter { $0.product.lowercased().contains(searchText.lowercased()) }
    }

    // Фильтрация продуктов по категории
    func filterProductsByCategory(category: String) -> [DogProductModel] {
        guard !category.isEmpty else { return getAllProducts() }
        return dogProduct.filter { $0.type.contains(category) }
    }
    
    // Сброс фильтрации
    func resetCategorySearchToAllProducts() -> [DogProductModel] {
